...
static HPDF_STATUS LoadFontData(HPDF_FontDef fontdef, HPDF_Stream stream, HPDF_BOOL embedding, HPDF_UINT offset);

static HPDF_STATUS LoadFontData2(HPDF_FontDef fontdef, HPDF_Stream stream, HPDF_UINT index, HPDF_BOOL embedding);

static void InitAttr(HPDF_FontDef fontdef);
...
static HPDF_STATUS WriteUINT32(HPDF_Stream stream, HPDF_UINT32 value);

static HPDF_STATUS WriteUINT16(HPDF_Stream stream, HPDF_UINT16 value);

static HPDF_STATUS WriteINT16(HPDF_Stream stream, HPDF_INT16 value);
...
static HPDF_STATUS LoadTTFTable(HPDF_FontDef fontdef);

static HPDF_STATUS ParseHead(HPDF_FontDef fontdef);

static HPDF_STATUS ParseMaxp(HPDF_FontDef fontdef);

static HPDF_STATUS ParseHhea(HPDF_FontDef fontdef);

static HPDF_STATUS ParseCMap(HPDF_FontDef fontdef);

static HPDF_STATUS ParseCMAP_format0(HPDF_FontDef fontdef, HPDF_UINT32 offset);

static HPDF_STATUS ParseCMAP_format4(HPDF_FontDef fontdef, HPDF_UINT32 offset);

static HPDF_STATUS ParseHmtx(HPDF_FontDef fontdef);

static HPDF_STATUS ParseLoca(HPDF_FontDef fontdef);

static HPDF_STATUS LoadUnicodeName(HPDF_Stream stream, HPDF_UINT offset, HPDF_UINT len, char* buf);

static HPDF_STATUS ParseName(HPDF_FontDef fontdef);

static HPDF_STATUS ParseOS2(HPDF_FontDef fontdef);

static HPDF_TTFTable* FindTable(HPDF_FontDef fontdef, const char* tag);

static void CleanFunc(HPDF_FontDef fontdef);

static HPDF_STATUS CheckCompositGryph(HPDF_FontDef fontdef, HPDF_UINT16 gid);
...
static void CleanFunc(HPDF_FontDef fontdef) {
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
    HPDF_MemSet(attr->glyph_tbl.flgs, 0, sizeof(HPDF_BYTE) * attr->num_glyphs);
...
static void DumpTable(HPDF_FontDef fontdef) {
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
    HPDF_Stream stream;
    HPDF_UINT i;
...
static HPDF_STATUS LoadFontData(HPDF_FontDef fontdef, HPDF_Stream stream, HPDF_BOOL embedding, HPDF_UINT offset) {
    ...
    fontdef->missing_width =
        (HPDF_INT16)((HPDF_UINT32)attr->h_metric[0].advance_width * 1000 / attr->header.units_per_em);
...
static HPDF_STATUS LoadFontData2(HPDF_FontDef fontdef, HPDF_Stream stream, HPDF_UINT index, HPDF_BOOL embedding) {
     ...
     * value.
     */
    if ((ret = HPDF_Stream_Seek(stream, 12 + index * 4, HPDF_SEEK_SET)) != HPDF_OK)
        return ret;
...
HPDF_Box HPDF_TTFontDef_GetCharBBox(HPDF_FontDef fontdef, HPDF_UINT16 unicode) {
    ...
    ret = HPDF_Stream_Seek(
        attr->stream, attr->glyph_tbl.base_offset + attr->glyph_tbl.offsets[gid] * m + 2, HPDF_SEEK_SET);
...
LoadTTFTable(HPDF_FontDef fontdef) {
    ...
    attr->offset_tbl.table = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTFTable) * attr->offset_tbl.num_tables);
    if (!attr->offset_tbl.table)
        return HPDF_Error_GetCode(fontdef->error);
...
static HPDF_STATUS ParseHead(HPDF_FontDef fontdef) {
    ...
    fontdef->font_bbox.left = (HPDF_REAL)((HPDF_INT32)attr->header.x_min * 1000 / attr->header.units_per_em);
    fontdef->font_bbox.bottom = (HPDF_REAL)((HPDF_INT32)attr->header.y_min * 1000 / attr->header.units_per_em);
    fontdef->font_bbox.right = (HPDF_REAL)((HPDF_INT32)attr->header.x_max * 1000 / attr->header.units_per_em);
    fontdef->font_bbox.top = (HPDF_REAL)((HPDF_INT32)attr->header.y_max * 1000 / attr->header.units_per_em);
...
static HPDF_STATUS ParseHhea(HPDF_FontDef fontdef) {
    ...
    fontdef->ascent = (HPDF_INT16)((HPDF_INT32)fontdef->ascent * 1000 / attr->header.units_per_em);
    ret += GetINT16(attr->stream, &fontdef->descent);
    fontdef->descent = (HPDF_INT16)((HPDF_INT32)fontdef->descent * 1000 / attr->header.units_per_em);
...
static HPDF_STATUS ParseCMap(HPDF_FontDef fontdef) {
    ...
    for (i = 0; i < num_cmap; i++) {
        ...
        ret = HPDF_Stream_Seek(attr->stream, tbl->offset + offset, HPDF_SEEK_SET);
        if (ret != HPDF_OK)
            return ret;
...
static HPDF_STATUS ParseCMAP_format0(HPDF_FontDef fontdef, HPDF_UINT32 offset) {
    ...
    attr->cmap.glyph_id_array = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * 256);
    if (!attr->cmap.glyph_id_array)
        return HPDF_Error_GetCode(fontdef->error);
...
static HPDF_STATUS ParseCMAP_format4(HPDF_FontDef fontdef, HPDF_UINT32 offset) {
    ...
    if ((ret = HPDF_Stream_Seek(attr->stream, offset, HPDF_SEEK_SET)) != HPDF_OK)
        return ret;
    ...
    /* end_count */
    attr->cmap.end_count = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
    if (!attr->cmap.end_count)
        return HPDF_Error_GetCode(fontdef->error);
    ...
    /* start_count */
    attr->cmap.start_count = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
    if (!attr->cmap.start_count)
        return HPDF_Error_GetCode(fontdef->error);
    ...
    /* id_delta */
    attr->cmap.id_delta = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
    if (!attr->cmap.id_delta)
        return HPDF_Error_GetCode(fontdef->error);
    ...
    /* id_range_offset */
    attr->cmap.id_range_offset = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.seg_count_x2 / 2);
    if (!attr->cmap.id_range_offset)
        return HPDF_Error_GetCode(fontdef->error);
    ...
    if (attr->cmap.glyph_id_array_count > 0) {
        /* glyph_id_array */
        attr->cmap.glyph_id_array = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT16) * attr->cmap.glyph_id_array_count);
        if (!attr->cmap.glyph_id_array)
            return HPDF_Error_GetCode(fontdef->error);
    ...
    for (i = 0; i < (HPDF_UINT)attr->cmap.seg_count_x2 / 2; i++) {
        HPDF_PTRACE((" ParseCMAP_format4[%d] start_count=0x%04X, "
                     ...
                     i,
                     attr->cmap.start_count[i],
                     attr->cmap.end_count[i],
                     attr->cmap.id_delta[i],
                     attr->cmap.id_range_offset[i]));
    }
#endif
...
HPDF_UINT16
HPDF_TTFontDef_GetGlyphid(HPDF_FontDef fontdef, HPDF_UINT16 unicode) {
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
    ...
    if (attr->cmap.id_range_offset[i] == 0) {
        ...
        HPDF_UINT idx = attr->cmap.id_range_offset[i] / 2 + (unicode - attr->cmap.start_count[i]) - (seg_count - i);

        if (idx > attr->cmap.glyph_id_array_count) {
            HPDF_PTRACE(
                (" HPDF_TTFontDef_GetGlyphid[%u] %u > %u\n", i, idx, (HPDF_UINT)attr->cmap.glyph_id_array_count));
            return 0;
        } else {
            HPDF_UINT16 gid = (HPDF_UINT16)(attr->cmap.glyph_id_array[idx] + attr->cmap.id_delta[i]);
...
HPDF_INT16
HPDF_TTFontDef_GetCharWidth(HPDF_FontDef fontdef, HPDF_UINT16 unicode) {
    ...
    advance_width = (HPDF_UINT16)((HPDF_UINT)hmetrics.advance_width * 1000 / attr->header.units_per_em);
...
static HPDF_STATUS CheckCompositGryph(HPDF_FontDef fontdef, HPDF_UINT16 gid) {
    ...
    if ((ret = HPDF_Stream_Seek(attr->stream, offset, HPDF_SEEK_SET)) != HPDF_OK) {
        ...
        if ((ret = HPDF_Stream_Seek(attr->stream, 8, HPDF_SEEK_CUR)) != HPDF_OK)
            return ret;
        ...
        do {
            ...
            if (flags & ARG_1_AND_2_ARE_WORDS) {
                if ((ret = HPDF_Stream_Seek(attr->stream, 4, HPDF_SEEK_CUR)) != HPDF_OK)
                    return ret;
            } else {
                if ((ret = HPDF_Stream_Seek(attr->stream, 2, HPDF_SEEK_CUR)) != HPDF_OK)
                    return ret;
            }
...
HPDF_INT16
HPDF_TTFontDef_GetGidWidth(HPDF_FontDef fontdef, HPDF_UINT16 gid) {
    HPDF_UINT16 advance_width;
    HPDF_TTF_LongHorMetric hmetrics;
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
...
static HPDF_STATUS ParseHmtx(HPDF_FontDef fontdef) {
     ...
     */
    attr->h_metric = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTF_LongHorMetric) * attr->num_glyphs);
    ...
    for (i = 0; i < attr->num_h_metric; i++) {
        if ((ret = GetUINT16(attr->stream, &pmetric->advance_width)) != HPDF_OK)
            return ret;

        if ((ret = GetINT16(attr->stream, &pmetric->lsb)) != HPDF_OK)
            return ret;

        HPDF_PTRACE((" ParseHmtx metric[%u] aw=%u lsb=%d\n", i, pmetric->advance_width, pmetric->lsb));
...
static HPDF_STATUS ParseLoca(HPDF_FontDef fontdef) {
    ...
    attr->glyph_tbl.offsets = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_UINT32) * (attr->num_glyphs + 1));

    if (!attr->glyph_tbl.offsets)
        return HPDF_Error_GetCode(fontdef->error);

    HPDF_MemSet(attr->glyph_tbl.offsets, 0, sizeof(HPDF_UINT32) * (attr->num_glyphs + 1));

    /* allocate glyph-flg-table.
     * this flgs are used to judge whether glyphs should be embedded.
     */
    attr->glyph_tbl.flgs = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_BYTE) * attr->num_glyphs);
...
static HPDF_STATUS LoadUnicodeName(HPDF_Stream stream, HPDF_UINT offset, HPDF_UINT len, char* buf) {
    ...
    if ((ret = HPDF_Stream_Read(stream, tmp, &len)) != HPDF_OK)
        return ret;
...
static HPDF_STATUS ParseName(HPDF_FontDef fontdef) {
    ...
    HPDF_PTRACE((" ParseName() format=%u, count=%u, string_offset=%u\n",
                 attr->name_tbl.format,
                 attr->name_tbl.count,
                 attr->name_tbl.string_offset));

    attr->name_tbl.name_records = HPDF_GetMem(fontdef->mmgr, sizeof(HPDF_TTF_NameRecord) * attr->name_tbl.count);
    ...
    for (i = 0; i < attr->name_tbl.count; i++) {
        ...
        if (name_rec->platform_id == 3 && name_rec->encoding_id == 1 && name_rec->name_id == 6 &&
            name_rec->language_id == 0x0409) {
            offset_id1u = tbl->offset + name_rec->offset + attr->name_tbl.string_offset;
            len_id1u = name_rec->length;
        }

        if (name_rec->platform_id == 3 && name_rec->encoding_id == 1 && name_rec->name_id == 2 &&
            name_rec->language_id == 0x0409) {
    ...
    if (offset_id1) {
        ...
        if ((ret = LoadUnicodeName(attr->stream, offset_id1u, len_id1u, attr->base_font)) != HPDF_OK)
            return ret;
    }
    ...
    if (offset_id2) {
        ...
        if ((ret = LoadUnicodeName(attr->stream, offset_id2u, len_id2u, tmp)) != HPDF_OK)
            return ret;
    }
...
static HPDF_STATUS ParseOS2(HPDF_FontDef fontdef) {
    ...
    if (attr->fs_type & (0x0002 | 0x0100 | 0x0200) && attr->embedding)
        return HPDF_SetError(fontdef->error, HPDF_TTF_CANNOT_EMBEDDING_FONT, 0);

    /* get fields sfamilyclass and panose. */
    if ((ret = HPDF_Stream_Seek(attr->stream, tbl->offset + 30, HPDF_SEEK_SET)) != HPDF_OK)
        return ret;
    ...
    if ((attr->sfamilyclass[0] > 0 && attr->sfamilyclass[0] < 6) || (attr->sfamilyclass[0] == 7))
...
static HPDF_STATUS RecreateGLYF(HPDF_FontDef fontdef, HPDF_UINT32* new_offsets, HPDF_Stream stream) {
    ...
    for (i = 0; i < attr->num_glyphs; i++) {
        ...
        if (attr->glyph_tbl.flgs[i] == 1) {
            ...
            while (len > 0) {
                HPDF_UINT tmp_len = (len > HPDF_STREAM_BUF_SIZ) ? HPDF_STREAM_BUF_SIZ : len;

                HPDF_MemSet(buf, 0, tmp_len);

                if ((ret = HPDF_Stream_Read(attr->stream, buf, &tmp_len)) != HPDF_OK)
                    return ret;

                if ((ret = HPDF_Stream_Write(stream, buf, tmp_len)) != HPDF_OK)
                    return ret;
...
static HPDF_STATUS RecreateName(HPDF_FontDef fontdef, HPDF_Stream stream) {
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
    HPDF_TTFTable* tbl = FindTable(fontdef, "name");
    HPDF_STATUS ret = HPDF_OK;
    HPDF_UINT i;
    HPDF_TTF_NameRecord* name_rec;
    HPDF_Stream tmp_stream = HPDF_MemStream_New(fontdef->mmgr, HPDF_STREAM_BUF_SIZ);
    ...
    for (i = 0; i < attr->name_tbl.count; i++) {
        ...
        HPDF_UINT offset = tbl->offset + attr->name_tbl.string_offset + name_rec->offset;
        ...
        while (tmp_len > 0) {
            HPDF_UINT len = (tmp_len > HPDF_STREAM_BUF_SIZ) ? HPDF_STREAM_BUF_SIZ : tmp_len;
    ...
    ret = HPDF_Stream_WriteToStream(tmp_stream, stream, HPDF_STREAM_FILTER_NONE, NULL);
...
HPDF_STATUS
HPDF_TTFontDef_SaveFontData(HPDF_FontDef fontdef, HPDF_Stream stream) {
    HPDF_TTFontDefAttr attr = (HPDF_TTFontDefAttr)fontdef->attr;
    ...
    for (i = 0; i < HPDF_REQUIRED_TAGS_COUNT; i++) {
        ...
        if (!tbl) {
            tbl = &emptyTable;
            HPDF_MemCpy((HPDF_BYTE*)tbl->tag, (const HPDF_BYTE*)REQUIRED_TAGS[i], 4);
        }
    ...
    for (i = 0; i < HPDF_REQUIRED_TAGS_COUNT; i++) {
        ...
        if ((ret = HPDF_Stream_Seek(tmp_stream, tbl.offset, HPDF_SEEK_SET)) != HPDF_OK)
            break;
...
void HPDF_TTFontDef_SetTagName(HPDF_FontDef fontdef, char* tag) {
    ...
    HPDF_MemCpy((HPDF_BYTE*)buf + HPDF_TTF_FONT_TAG_LEN + 1,
                (HPDF_BYTE*)fontdef->base_font,
...
static void UINT32Swap(HPDF_UINT32* value) {
    ...
    *value =
        (HPDF_UINT32)((HPDF_UINT32)b[0] << 24 | (HPDF_UINT32)b[1] << 16 | (HPDF_UINT32)b[2] << 8 | (HPDF_UINT32)b[3]);
}
