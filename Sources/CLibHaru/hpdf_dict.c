...
HPDF_DictElement GetElement(HPDF_Dict dict, const char* key);
...
void HPDF_Dict_Free(HPDF_Dict dict) {
    ...
    for (i = 0; i < dict->list->count; i++) {
        HPDF_DictElement element = (HPDF_DictElement)HPDF_List_ItemAt(dict->list, i);
...
HPDF_Dict_Add_FilterParams(HPDF_Dict dict, HPDF_Dict filterParam) {
    ...
    paramArray = HPDF_Dict_GetItem(dict, "DecodeParms", HPDF_OCLASS_ARRAY);
    if (paramArray == NULL) {
...
HPDF_Dict_Write(HPDF_Dict dict, HPDF_Stream stream, HPDF_Encrypt e) {
    ...
    if (dict->stream) {
        ...
        if (dict->filter == HPDF_STREAM_FILTER_NONE)
            ...
            HPDF_Array array = HPDF_Dict_GetItem(dict, "Filter", HPDF_OCLASS_ARRAY);
...
HPDF_Dict_Add(HPDF_Dict dict, const char* key, void* obj) {
    ...
    if (element) {
        ...
        element = (HPDF_DictElement)HPDF_GetMem(dict->mmgr, sizeof(HPDF_DictElement_Rec));
...
void* HPDF_Dict_GetItem(HPDF_Dict dict, const char* key, HPDF_UINT16 obj_class) {
    ...
    if (element && HPDF_StrCmp(key, element->key) == 0) {
        ...
        if ((header->obj_class & HPDF_OCLASS_ANY) != obj_class) {
            HPDF_PTRACE(
                (" HPDF_Dict_GetItem dict=%p key=%s obj_class=0x%08X\n", dict, key, (HPDF_UINT)header->obj_class));
