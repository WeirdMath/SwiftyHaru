...
#define HEADER                                                                                                         \
    "<?xpacket begin='' id='W5M0MpCehiHzreSzNTczkc9d'?><x:xmpmeta xmlns:x='adobe:ns:meta/' x:xmptk='XMP toolkit "      \
    "2.9.1-13, framework 1.6'><rdf:RDF xmlns:rdf='http://www.w3.org/1999/02/22-rdf-syntax-ns#' "                       \
...
HPDF_PDFA_SetPDFAConformance(HPDF_Doc pdf, HPDF_PDFAType pdfatype) {
    ...
    if ((dc_title != NULL) || (dc_creator != NULL) || (dc_description != NULL) || (xmp_CreateDate != NULL) ||
        (xmp_ModifyDate != NULL) || (xmp_CreatorTool != NULL) || (pdf_Keywords != NULL)) {
