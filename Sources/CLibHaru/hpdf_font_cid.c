...
static HPDF_Font CIDFontType0_New(HPDF_Font parent, HPDF_Xref xref);
...
static HPDF_UINT MeasureText(HPDF_Font font,
...
static char* UINT16ToHex(char* s, HPDF_UINT16 val, char* eptr, HPDF_BYTE width);

static char* CidRangeToHex(char* s, HPDF_UINT16 from, HPDF_UINT16 to, char* eptr);

static HPDF_Dict CreateCMap(HPDF_Encoder encoder, HPDF_Xref xref);

static void OnFree_Func(HPDF_Dict obj);
...
HPDF_Font HPDF_Type0Font_New(HPDF_MMgr mmgr, HPDF_FontDef fontdef, HPDF_Encoder encoder, HPDF_Xref xref) {
    ...
    if (!attr->descendant_font)
        return NULL;
    else if (HPDF_Array_Add(descendant_fonts, attr->descendant_font) != HPDF_OK)
        return NULL;
...
static HPDF_STATUS CIDFontType2_BeforeWrite_Func(HPDF_Dict obj) {
    ...
    if ((ret = HPDF_Dict_AddName(obj, "BaseFont", def_attr->base_font)) != HPDF_OK)
        return ret;

    if ((ret = HPDF_Dict_AddName(font_attr->descendant_font, "BaseFont", def_attr->base_font)) != HPDF_OK)
        return ret;
...
static HPDF_TextWidth TextWidth(HPDF_Font font, const HPDF_BYTE* text, HPDF_UINT len) {
    ...
    if (attr->fontdef->type == HPDF_FONTDEF_TYPE_CID) {
        ...
        dw2 = (HPDF_INT)(attr->fontdef->font_bbox.bottom - attr->fontdef->font_bbox.top);
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (!wordwrap) {
            if (HPDF_IS_WHITE_SPACE(b)) {
                ...
                if (real_width)
                    *real_width = w;
            } else if (btype == HPDF_BYTE_TYPE_SINGLE || btype == HPDF_BYTE_TYPE_LEAD) {
                tmp_len = i;
                if (real_width)
                    *real_width = w;
            }
        ...
        if (btype != HPDF_BYTE_TYPE_TRIAL) {
            if (attr->writing_mode == HPDF_WMODE_HORIZONTAL) {
                if (attr->fontdef->type == HPDF_FONTDEF_TYPE_CID) {
                    ...
                    /* unicode-based font */
                    unicode = (encoder->to_unicode_fn)(encoder, code);
                    tmp_w = HPDF_TTFontDef_GetCharWidth(attr->fontdef, unicode);
                }
            } else {
