...
static const char* const HPDF_INHERITABLE_ENTRIES[5] = {"Resources", "MediaBox", "CropBox", "Rotate", NULL};
...
HPDF_STATUS
HPDF_Pages_AddKids(HPDF_Pages parent, HPDF_Dict kid) {
    HPDF_Array kids;
    HPDF_STATUS ret;
...
HPDF_STATUS
HPDF_Page_InsertBefore(HPDF_Page page, HPDF_Page target) {
    HPDF_Page parent;
    HPDF_Array kids;
    HPDF_STATUS ret;
    HPDF_PageAttr attr;
...
HPDF_STATUS
HPDF_Page_CheckState(HPDF_Page page, HPDF_UINT mode) {
    if (!page)
...
const char* HPDF_Page_GetLocalFontName(HPDF_Page page, HPDF_Font font) {
    ...
    if (!attr->fonts) {
        ...
        resources = HPDF_Page_GetInheritableItem(page, "Resources", HPDF_OCLASS_DICT);
        if (!resources)
            return NULL;
...
HPDF_UINT HPDF_Page_MeasureText(HPDF_Page page,
    ...
    ret = HPDF_Font_MeasureText(attr->gstate->font,
                                (HPDF_BYTE*)text,
                                len,
                                width,
...
HPDF_REAL HPDF_Page_GetTextRaise(HPDF_Page page) { return HPDF_Page_GetTextRise(page); }
...
HPDF_Annotation
    HPDF_Page_CreateProjectionAnnot(HPDF_Page page, HPDF_Rect rect, const char* text, HPDF_Encoder encoder) {
    HPDF_PageAttr attr;
    HPDF_Annotation annot;
