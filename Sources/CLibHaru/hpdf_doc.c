...
static const char* const HPDF_VERSION_STR[6] = {"%PDF-1.2\012%\267\276\255\252\012",
...
static const char*
    LoadTTFontFromStream(HPDF_Doc pdf, HPDF_Stream font_data, HPDF_BOOL embedding, const char* file_name);
...
const char* HPDF_GetVersion(void) { return HPDF_VERSION_TEXT; }
...
HPDF_STATUS HPDF_NewDoc(HPDF_Doc pdf) {
    ...
    if (!pdf->fontdef_list) {
        pdf->fontdef_list = HPDF_List_New(pdf->mmgr, HPDF_DEF_ITEMS_PER_BLOCK);
        if (!pdf->fontdef_list)
            return HPDF_CheckError(&pdf->error);
    }

    if (!pdf->encoder_list) {
        pdf->encoder_list = HPDF_List_New(pdf->mmgr, HPDF_DEF_ITEMS_PER_BLOCK);
        if (!pdf->encoder_list)
            return HPDF_CheckError(&pdf->error);
    }
...
HPDF_Doc_SetEncryptOn(HPDF_Doc pdf) {
    ...
    if (!pdf->encrypt_dict)
        return HPDF_SetError(&pdf->error, HPDF_DOC_ENCRYPTDICT_NOT_FOUND, 0);
...
HPDF_STATUS HPDF_SetPassword(HPDF_Doc pdf, const char* owner_passwd, const char* user_passwd) {
    ...
    if (HPDF_EncryptDict_SetPassword(pdf->encrypt_dict, owner_passwd, user_passwd) != HPDF_OK)
        return HPDF_CheckError(&pdf->error);
...
HPDF_STATUS HPDF_SetPermission(HPDF_Doc pdf, HPDF_UINT permission) {
    ...
    if (!e)
        return HPDF_RaiseError(&pdf->error, HPDF_DOC_ENCRYPTDICT_NOT_FOUND, 0);
    else
    ...
    if (!e)
        ...
        if (mode == HPDF_ENCRYPT_R2)
            ...
            if (key_len >= 5 && key_len <= 16)
                ...
                return HPDF_RaiseError(&pdf->error, HPDF_INVALID_ENCRYPT_KEY_LEN, 0);
        }
        e->mode = mode;
    }
...
HPDF_Doc_PrepareEncryption(HPDF_Doc pdf) {
    ...
    if (HPDF_EncryptDict_Prepare(pdf->encrypt_dict, info, pdf->xref) != HPDF_OK)
        return pdf->error.error_no;
...
HPDF_STATUS
HPDF_Doc_SetCurrentPages(HPDF_Doc pdf, HPDF_Pages pages) {
...
HPDF_STATUS
HPDF_Doc_SetCurrentPage(HPDF_Doc pdf, HPDF_Page page) {
    ...
    for (i = 0; i < list->count; i++) {
        ...
        if (HPDF_StrCmp(font_name, def->base_font) == 0) {
            if (def->type == HPDF_FONTDEF_TYPE_UNINITIALIZED) {
                if (!def->init_fn || def->init_fn(def) != HPDF_OK)
                    return NULL;
            }
...
HPDF_STATUS
HPDF_Doc_RegisterFontDef(HPDF_Doc pdf, HPDF_FontDef fontdef) {
    HPDF_STATUS ret;
    ...
    for (i = 0; i < list->count; i++) {
        ...
        if (HPDF_StrCmp(encoding_name, encoder->name) == 0) {
            ...
            if (encoder->type == HPDF_ENCODER_TYPE_UNINITIALIZED) {
                if (!encoder->init_fn || encoder->init_fn(encoder) != HPDF_OK)
                    return NULL;
            }
...
HPDF_STATUS
HPDF_Doc_RegisterEncoder(HPDF_Doc pdf, HPDF_Encoder encoder) {
    HPDF_STATUS ret;
    ...
    if (!encoding_name) {
        ...
        if (fontdef) {
            ...
            if (fontdef->type == HPDF_FONTDEF_TYPE_TYPE1 &&
                HPDF_StrCmp(attr->encoding_scheme, HPDF_ENCODING_FONT_SPECIFIC) == 0)
...
const char* HPDF_LoadType1FontFromFile(HPDF_Doc pdf, const char* afm_file_name, const char* data_file_name) {
    ...
    if (HPDF_Stream_Validate(afm) && (!data_file_name || HPDF_Stream_Validate(pfm))) {
...
const char* HPDF_LoadTTFontFromMemory(HPDF_Doc pdf, const HPDF_BYTE* buffer, HPDF_UINT size, HPDF_BOOL embedding) {
    HPDF_Stream font_data;
    const char* ret;
...
const char* HPDF_LoadTTFontFromMemory2(HPDF_Doc pdf,
...
HPDF_Image HPDF_LoadRawImageFromFile(HPDF_Doc pdf,
    ...
    if (HPDF_Stream_Validate(imagedata))
        image = HPDF_Image_LoadRawImage(pdf->mmgr, imagedata, pdf->xref, width, height, color_space);
    else
        image = NULL;
...
HPDF_STATUS HPDF_AddPageLabel(HPDF_Doc pdf,
    ...
    if (style >= HPDF_PAGE_NUM_STYLE_EOF)
...
HPDF_Outline HPDF_CreateOutline(HPDF_Doc pdf, HPDF_Outline parent, const char* title, HPDF_Encoder encoder) {
    ...
    if (!parent) {
        if (pdf->outlines) {
            ...
            if (pdf->outlines) {
                HPDF_STATUS ret = HPDF_Dict_Add(pdf->catalog, "Outlines", pdf->outlines);
                if (ret != HPDF_OK) {
                    HPDF_CheckError(&pdf->error);
                    pdf->outlines = NULL;
                    return NULL;
 ...
 */
HPDF_OutputIntent HPDF_OutputIntent_New(HPDF_Doc pdf,
...
HPDF_OutputIntent
    HPDF_ICC_LoadIccFromMem(HPDF_Doc pdf, HPDF_MMgr mmgr, HPDF_Stream iccdata, HPDF_Xref xref, int numcomponent) {
