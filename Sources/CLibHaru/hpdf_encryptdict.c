...
void HPDF_EncryptDict_CreateID(HPDF_EncryptDict dict, HPDF_Dict info, HPDF_Xref xref) {
    ...
    if (info) {
        ...
        HPDF_MD5Update(&ctx, (const HPDF_BYTE*)&(xref->entries->count), sizeof(HPDF_UINT32));
    }
#endif
...
HPDF_STATUS
HPDF_EncryptDict_Prepare(HPDF_EncryptDict dict, HPDF_Dict info, HPDF_Xref xref) {
    HPDF_STATUS ret;
    HPDF_Encrypt attr = (HPDF_Encrypt)dict->attr;
...
HPDF_STATUS
HPDF_EncryptDict_SetPassword(HPDF_EncryptDict dict, const char* owner_passwd, const char* user_passwd) {
    HPDF_Encrypt attr = (HPDF_Encrypt)dict->attr;
