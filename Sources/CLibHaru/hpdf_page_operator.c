...
static HPDF_STATUS InternalArc(HPDF_Page page,
                               HPDF_REAL x,
                               HPDF_REAL y,
                               HPDF_REAL ray,
                               HPDF_REAL ang1,
...
/* Ts */
HPDF_STATUS HPDF_Page_SetTextRaise(HPDF_Page page, HPDF_REAL value) { return HPDF_Page_SetTextRise(page, value); }
...
HPDF_STATUS HPDF_Page_SetRGBFill(HPDF_Page page, HPDF_REAL r, HPDF_REAL g, HPDF_REAL b) {
    HPDF_STATUS ret = HPDF_Page_CheckState(page, HPDF_GMODE_TEXT_OBJECT | HPDF_GMODE_PAGE_DESCRIPTION);
    char buf[HPDF_TMP_BUF_SIZ];
    char* pbuf = buf;
    char* eptr = buf + HPDF_TMP_BUF_SIZ - 1;
    HPDF_PageAttr attr;
...
static HPDF_STATUS InternalWriteText(HPDF_PageAttr attr, const char* text) {
    ...
    if (font_attr->type == HPDF_FONT_TYPE0_TT || font_attr->type == HPDF_FONT_TYPE0_CID) {
        ...
        if (encoder->encode_text_fn == NULL) {
            if ((ret = HPDF_Stream_WriteBinary(attr->stream, (HPDF_BYTE*)text, len, NULL)) != HPDF_OK)
                return ret;
        } else {
            char* encoded;
            HPDF_UINT length;
 ...
 */
static void
    TextPos_AbsToRel(HPDF_TransMatrix text_matrix, HPDF_REAL xAbs, HPDF_REAL yAbs, HPDF_REAL* xRel, HPDF_REAL* yRel) {
    if (text_matrix.a == 0) {
        ...
        *yRel = (xAbs - text_matrix.x) / text_matrix.c;
        ...
        HPDF_REAL y = (yAbs - text_matrix.y - (xAbs - text_matrix.x) * text_matrix.b / text_matrix.a) /
                      (text_matrix.d - text_matrix.c * text_matrix.b / text_matrix.a);
