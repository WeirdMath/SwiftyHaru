...
static const HPDF_BYTE HPDF_PADDING_STRING[] = {0x28, 0xBF, 0x4E, 0x5E, 0x4E, 0x75, 0x8A, 0x41, 0x64, 0x00, 0x4E,
...
#define HPDF_MD5STEP(f, w, x, y, z, data, s) (w += f(x, y, z) + data, w = w << s | w >> (32 - s), w += x)
...
void HPDF_Encrypt_Init(HPDF_Encrypt attr) {
    ...
    attr->permission =
        HPDF_ENABLE_PRINT | HPDF_ENABLE_EDIT_ALL | HPDF_ENABLE_COPY | HPDF_ENABLE_EDIT | HPDF_PERMISSION_PAD;
}
...
void ARC4Init(HPDF_ARC4_Ctx_Rec* ctx, const HPDF_BYTE* key, HPDF_UINT key_len) {
...
void ARC4CryptBuf(HPDF_ARC4_Ctx_Rec* ctx, const HPDF_BYTE* in, HPDF_BYTE* out, HPDF_UINT len) {
    HPDF_UINT i;
    HPDF_UINT t;
    HPDF_BYTE K;
