...
static const char* const HPDF_BM_NAMES[] = {"Normal",
                                            "Multiply",
                                            "Screen",
                                            "Overlay",
                                            "Darken",
...
HPDF_ExtGState_Validate(HPDF_ExtGState ext_gstate) {
    if (!ext_gstate || (ext_gstate->header.obj_class != (HPDF_OSUBCLASS_EXT_GSTATE | HPDF_OCLASS_DICT) &&
...
ExtGState_Check(HPDF_ExtGState ext_gstate) {
    ...
    if (ext_gstate->header.obj_class == (HPDF_OSUBCLASS_EXT_GSTATE_R | HPDF_OCLASS_DICT))
        return HPDF_RaiseError(ext_gstate->error, HPDF_EXT_GSTATE_READ_ONLY, 0);
...
HPDF_STATUS HPDF_ExtGState_SetAlphaStroke(HPDF_ExtGState ext_gstate, HPDF_REAL value) {
    ...
    if (value < 0 || value > 1.0f)
        return HPDF_RaiseError(ext_gstate->error, HPDF_EXT_GSTATE_OUT_OF_RANGE, 0);
